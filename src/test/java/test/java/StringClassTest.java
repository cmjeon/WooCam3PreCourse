...
public class StringClassTest {
	 ...
	 * 2. "1"을 ,로 split 했을 때 1만을 포함하는 배열이 반환되는지에 대한 학습테스트를 구현한다.
	 */
	@Test
	...
	/*
	 * 요구사항2 
	 * 1. "(1,2)" 값이 주어졌을 때 String의 substring() 메소드를 확용해 ()을 제거하고 "1,2"를 반환하도록 구현한다.
	 */
	@Test
	 ...
	 * 2. String의 charAt() 메소드를 활용해 특정 위치의 문자를 가져올 때 위치 값을 벗어나면 StringIndexOutOfBoundsException이 발생하는 부분에 대한 학습 테스트를 구현한다.
	...
	public void getCharFromStringAssertions() {
		StringClass sc = new StringClass();
		assertThat(sc.getCharAtFromString("abc", 0)).isEqualTo('a');
		assertThat(sc.getCharAtFromString("abc", 1)).isEqualTo('b');
		assertThat(sc.getCharAtFromString("abc", 2)).isEqualTo('c');
		assertThatThrownBy(() -> sc.getCharAtFromString("abc", 3)).isInstanceOf(IndexOutOfBoundsException.class).hasMessageContaining("Index: 3, Size: 3");
	}
